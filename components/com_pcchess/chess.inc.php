...
global $move_from_Square,$move_to_Square,$move_promotion_figur,$move_ep_Square,$move_turn,
       $move_castling_ws,$move_castling_wl,$move_castling_bs,$move_castling_bl,$move_PlyNumber,
       $move_list_from,$move_list_to,$move_list_count,$pgn_piece,$pgn_from_digit,$pgn_from_letter,
       $pgn_capture,$pgn_square_list,$State,$ab_Bishop,$ab_Rook,$ab_Knight,$ab_Queen,$ab_King,
       $Board,$Notation, $NotationList, $pgn_Move,$first_Position,$empty_Position;
...
# Legal from square?
#echo "DBG: is_Move_Possible $f<br>";
...
#echo "DBG: execute_Move From $move_from_Square To $move_to_Square<br>";
...
function set_Move_Vars($aMove) {
    ...
    #echo "DBG: Move From $move_from_Square To $move_to_Square<br>";
    $move_promotion_figur = 0;
...
function add_Move($Move) {
		...
		#echo "DBG: Hard to die ".$Move." ". set_Move_Vars($Move)."<br>";
                return FALSE;
	}
	execute_Move();
...
# BTW, the JavaScripts assigned to the squares need an html input text field with the name "pcc_Move" in a form named "pcc_data" as available on the demo pages.
        ...
        $fa  = "<font color=\"#000000\">";  //999     //цвет символов вокруг доски
        $fe  = "</font>";
        $dc  = "\"#f6f6f6\""; //f5f5f5  //фон вокруг доски
        $k   = "abcdefgh";
        $ds  = "\"#ffffff\"";
		$dz  = "";

        $s = "<table " . ($showaddmoveform ? "style=\"cursor:pointer\" " : '') .
		 "class=\"table_out\"><tbody><tr><td><table border=\"0\" cellpadding=\"1\" cellspacing=\"0\"><tbody>";
# Coordinates
...
#
		...
		if ($ds == "\"#135cae\"") {   // c68c53 135cae  // темные           //цвет клеток доски
                     $ds = "\"#e5e5e5\"";  //f8e1b8   //светлые
                }
		else {
                     $ds = "\"#135cae\"";  //c68c53
                }
...
# /images is path to pictures of pieces
                        ...
                        $x = ($x >> 3) * 10 + ((($x + 8) % 8)) + 21;
                        if      ($Board[$x] == WK) $si = "alpha/wk.png";
                        else if ($Board[$x] == WQ) $si = "alpha/wq.png";
                        else if ($Board[$x] == WR) $si = "alpha/wr.png";
                        else if ($Board[$x] == WB) $si = "alpha/wb.png";
                        else if ($Board[$x] == WN) $si = "alpha/wn.png";
                        else if ($Board[$x] == WP) $si = "alpha/wp.png";
                        else if ($Board[$x] == BK) $si = "alpha/bk.png";
                        else if ($Board[$x] == BQ) $si = "alpha/bq.png";
                        else if ($Board[$x] == BR) $si = "alpha/br.png";
                        else if ($Board[$x] == BB) $si = "alpha/bb.png";
                        else if ($Board[$x] == BN) $si = "alpha/bn.png";
                        else if ($Board[$x] == BP) $si = "alpha/bp.png";
                        else                       $si = "fleer.gif";
			$dz = $dz . "src=\"" . JUri::base(true) . "/components/com_pcchess/images/" . $si .
			 "\" height=\"45\" width=\"45\" alt=\"" . $Board[$x] . "\"/></td>\n"; // new square dimension (45x45)
			$s = $s . $dz;
		}
                $l = Abs($i - 8);
                if ($BlackBelow) $l = Abs($l - 9);
